...
Spring runs a Component Scan to check for all the Beans that it manages. If you do not give any arguments to the ```@ComponentScan``` annotation, it will only check for Beans in the current package (here it is ```io.datajek.spring.basics.movierecommendersystem.lesson1```). Any Beans outside that package will not be detected by defauly, unless we give it the ```@ComponentScan``` annotation.
